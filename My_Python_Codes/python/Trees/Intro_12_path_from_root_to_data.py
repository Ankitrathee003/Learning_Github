class BinaryTreeNode:
    ...
    def Path_root_to_Node(root,s):
        ...
        if root.data==s:
            ...
            l.append(root.data)
            return l
        leftoutput=BinaryTreeNode.Path_root_to_Node(root.left,s)
        if leftoutput!=None:
            leftoutput.append(root.data)
            return leftoutput
        rightoutput=BinaryTreeNode.Path_root_to_Node(root.right,s)
        if rightoutput!=None:
...
print(f"path from root to Node is {BinaryTreeNode.Path_root_to_Node(root,9)}")
